...
- **任务标记** — 为每个番茄钟标记正在做的事，完成后可编辑
- **手动完成** — 跳过时按实际专注时间记录，给予对应生长阶段奖励
- **放弃本次** — 计时中可放弃，不计入记录

### 番茄生长系统 🌱→🍅
根据专注时长显示不同生长阶段：
- 🌱 发芽（<10min）→ 🌿 幼苗（10-14min）→ 🌸 开花（15-19min）→ 🫒 青果（20-24min）→ 🍅 成熟（≥25min）

### 庆祝动画
- 番茄钟完成时弹跳 emoji + 粒子纸屑 + 进度环脉冲 + 鼓励文字
- 红番茄（≥25min）庆祝效果更强烈
...
│   ├── CelebrationOverlay.tsx # 完成庆祝动画（粒子 + 弹跳 emoji）
│   ├── TaskInput.tsx          # 任务输入框

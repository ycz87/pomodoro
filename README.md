...
**当前版本：** v0.31.0（2026-02-19）

## 功能

### 导航
- **四 Tab 导航** — 🍉专注 / 🏠瓜棚 / 🌱农场 / 🏪商城，顶栏 segmented control 切换
...
- **变异射线枪** — 对单株使用后变异概率 +20%，最多 5 次叠加到 100%

### 商城系统 🏪
- **瓜币系统** — 独立余额存储（`watermelon-coins`），localStorage 持久化
- **顶部余额展示** — 商城页右上角实时显示 `💰 {balance}`
- **买/卖双 Tab** — 买入区（道具 + 地块扩展）与卖出区都可用
- **买道具** — 14 种常驻商品，购买后立即写入背包道具存储
- **购买确认弹窗** — 下单前显示道具名、价格、当前余额
- **地块购买区** — 可购买第 5-9 块地（200 / 500 / 1000 / 2000 / 5000 瓜币）
- **已解锁状态** — 已购地块显示 `✅ 已解锁`，按钮禁用避免重复购买
- **卖瓜流程** — 选择品种 → 二次确认 → 图鉴 count-1 → 瓜币 +sellPrice
- **变异体售价** — `isMutant` 变异体按原品种 sellPrice 的 3 倍出售
- **图鉴保留** — count 降到 0 仍保留图鉴条目
- **首次收获奖励** — 新品种首次收获额外获得等额 sellPrice 瓜币
- **70 品种售价表** — 40 纯种 + 30 杂交全部配置 sellPrice
